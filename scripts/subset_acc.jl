...
@everywhere function obtain_rates(ref, estimated)
    ...
    res = read(run(`python3 $fastsp_script -r $ref -e $estimated`), String)
...
for arg in ARGS
    for p in glob(arg)
        ...
        subset_path = joinpath(p, "decomposition")
        subset_alignment_pattern = joinpath(subset_path, "subset_*.txt")
